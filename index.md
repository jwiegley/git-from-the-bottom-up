...
* **remote** — This is a reference to a different copy of the same repository located elsewhere, e.g. on another computer. The **remote** repository may be in a different state to the local copy but should always share at least one common ancestor commit. That the local and remote repositories can diverge from each other is what allows `git` to enable collaborative distributed work.
